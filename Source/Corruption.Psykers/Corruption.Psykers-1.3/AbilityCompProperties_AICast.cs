...
namespace Corruption.Psykers
{
    public class AbilityComp_AICast : AbilityComp
    {
        ...
        private bool Recklessness()
        {
            ...
            {
                if (parent.pawn.health.hediffSet.GetHediffs<HediffDemonicAttention>().Any(x => x.def.maxSeverity <= x.Severity + this.parent.def.EntropyGain))
                {
                    return false;
                }
            }
